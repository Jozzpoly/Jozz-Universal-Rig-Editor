...
function rigHasFrame(rig: RigDocument, id: string): boolean {
  return rig.frames.some((frame) => frame.id === id);
}

function rigHasDatum(rig: RigDocument, datum: { kind: 'element' | 'frame'; id: string }): boolean {
  return datum.kind === 'element'
    ? rig.elements.some((element) => element.id === datum.id)
    : rigHasFrame(rig, datum.id);
}
...
export function validateJureProjectModel(project: JureProjectModel): Diagnostic[] {
  ...
  for (const adoption of project.sourceAdoptions) {
    ...
    if (!rig) {
      diagnostics.push({ code: 'project.adoption.document.missing', severity: 'error', message: `Source adoption ${adoption.id} references missing rig document ${adoption.target.documentId}.`, references: [adoption.id, adoption.target.documentId] });
      continue;
    }
    const target = adoption.target.kind === 'element'
      ? rig.elements.find((element) => element.id === adoption.target.id)
      : rig.frames.find((frame) => frame.id === adoption.target.id);
    if (!target) {
  ...
  for (const representation of representationDocuments) {
    diagnostics.push(...validateRepresentationProjectReferences(representation, rigById, instanceById, sourceById));
  }
